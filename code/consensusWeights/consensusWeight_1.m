...
data = struct('y', y, 'x', x, 'g', g, 'totalTrials', totalTrials, 'nGroups', nGroups);
...
set(F, ...
    'color'             , 'w'               , ...
    'units'             , 'normalized'      , ...
    'position'          , [0.2 0.2 0.55 0.6] , ...
    'papersize'         , [11 6.75]         , ...
    'paperpositionmode' , 'auto'            );

% groups in subplots
[nRows, nCols] = subplotArrange(nGroups);
yPostpred = nan(totalTrials, 3); % mean, lower, upper
yPostpred(:, 1) = get_matrix_from_coda(chains, 'yPostpred', @mean);
for idx = 1:totalTrials
 tmp = prctile(chains.(sprintf('yPostpred_%d', idx))(:), [(100-CI)/2 100-(100-CI)/2]);
    yPostpred(idx, 2) = tmp(1);
    yPostpred(idx, 3) = tmp(2);
end

for groupIdx = 1:nGroups
    subplot(nRows, nCols, groupIdx); hold on;
    % axis
    set(gca, ...
        'xlim'       , [0 50]                , ...
        'xtick'      , 0:10:50               , ...
    ...
    % label
    text(0, 50, sprintf(' Group %d', groupIdx), ...
        'vertical' , 'mid'      , ...
        'fontsize' , fontSize   );
    
    % plot
    plot([0 50], [0 50], 'k-');
    match = find(g == groupIdx);
    for idx2 = 1:length(match)
        plot(ones(1, 2)*y(match(idx2)), yPostpred(match(idx2), [2 3]), 'k-', ...
        'color', pantone.Titanium);
    end
    plot(y(match), yPostpred(match, 1),'o', ...
        'markerfacecolor', pantone.ClassicBlue , ...
        'markeredgecolor', 'w'                 , ...
        'markersize'     , 8                   );
    Raxes(gca, 0.0085, 0.005);
end

% labels
[gca, H(1)] = suplabel('Behavioral Data', 'x');
[gca, H(2)] = suplabel('Model Estimate', 'y');
...
if printFigures
    warning off;
    print(sprintf('figures/%s_%s_descriptiveAdequacy.png', modelName, dataName), '-dpng');
    print(sprintf('figures/%s_%s_descriptiveAdequacy.pdf', modelName, dataName), '-dpdf');
    warning on;
end
